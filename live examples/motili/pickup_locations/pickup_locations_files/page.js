...
$(document).ready(function() {
    ...
    ***************************************************/
                              if($(".registria_field:input[name*='location[address_attributes][county]']").exists()){
          $(".registria_field:input[name*='location[address_attributes][county]']").rules("add", {
            required: true
          });
        }
                                    if($(".registria_field:input[name*='location[phone]']").exists()) {
          $(".registria_field:input[name*='location[phone]']").rules("add", {
          ...
          $(".registria_field:input[name*='location[phone]']").rules("add", {
            required: true
                                          ...
                                          if($(".registria_field:input[name*='location[address_attributes][zip]']").exists()){
          $(".registria_field:input[name*='location[address_attributes][zip]']").rules("add", {
            required: true
          });
        }
                                          if($(".registria_field:input[name*='location[address_attributes][country]']").exists()){
          $(".registria_field:input[name*='location[address_attributes][country]']").rules("add", {
            required: true
          });
        }
                                          if($(".registria_field:input[name*='location[address_attributes][state]']").exists()){
          $(".registria_field:input[name*='location[address_attributes][state]']").rules("add", {
            required: true
          });
        }
                                          if($(".registria_field:input[name*='location[address_attributes][city]']").exists()){
          $(".registria_field:input[name*='location[address_attributes][city]']").rules("add", {
            required: true
          });
        }
                                          if($(".registria_field:input[name*='location[address_attributes][address2]']").exists()){
          $(".registria_field:input[name*='location[address_attributes][address2]']").rules("add", {
            required: true
          });
        }
                                          if($(".registria_field:input[name*='location[address_attributes][address1]']").exists()){
          $(".registria_field:input[name*='location[address_attributes][address1]']").rules("add", {
            required: true
          });
        }
                                          if($(".registria_field:input[name*='location[preference]']").exists()){
          $(".registria_field:input[name*='location[preference]']").rules("add", {
            required: true
          });
        }
                                          if($(".registria_field:input[name*='location[site]']").exists()){
          $(".registria_field:input[name*='location[site]']").rules("add", {
            required: true
          });
        }
                                          if($(".registria_field:input[name*='location[name]']").exists()){
          $(".registria_field:input[name*='location[name]']").rules("add", {
            required: true
          });
        }
...
/* ===== Revised by Brady to allow multiple tables per page ===== */
...
function setPreferredLocation(locationData) {
  $('#location_name').val(locationData.name);
  $('#location_address1').val(locationData.address1);
  $('#location_address2').val(locationData.address2);
  $('#location_city').val(locationData.city);
  $('#location_state').val(locationData.state);
  $('#location_zip').val(locationData.zip);
  $('#registria_form').submit();
}

function locationLookup(zip) {
  $.ajax({
    type: 'GET',
    url: '/mmc/installation_locations',
    data: {
      zip: zip
    },
    dataType: 'json',
    success: function (data, status){
      ...
      if (data.locations != '') {
        $.each(data.locations, function(i, v){
          var l = data.locations[i].location;
          location_list += '<tr><td><span data-attr="location_name" data-value="'+ l.name +'"><strong>' + l.name + '</strong></span><br><span data-attr="location_phone" data-value="'+ (l.phone || "") +'"><small>' + (l.address.phone || "") + '</small></span> </td><td><span data-attr="location_address1" data-value="'+ l.address.address1 +'">' + l.address.address1 + '</span><br><span data-attr="location_city" data-value="'+ l.address.city +'">' + l.address.city + '</span>, <span data-attr="location_state" data-value="'+ l.address.state +'">' + l.address.state + '</span> <span data-attr="location_zip" data-value="'+ l.address.zip +'">' + l.address.zip + '</span></td><td data-select-preferred-location><span class="btn btn-primary pull-right">Add</span></td></tr>';
        });
        $('#location_data').html(location_list);
        $('#zipcode_table').show();
        $('.text-center').hide();
        $('#location-default-table').hide();
        $('#reset-zip-search').show();
      } else {
        location_list += '<tr><td colspan="7">No locations found</td></tr>';
        $('#location_data').html(location_list);
        $('#zipcode_table').show();
        $('.text-center').show();
        $('#location-default-table').show();
      }
    }
  }).fail(function(e) {
    var location_list = '<tr><td colspan="7">No locations found</td></tr>';
    $('#location_data').html(location_list);
    $('#zipcode_table').show();
  }); 
}
...
$(document).ready(function() {
  ...
  // do not allow non-admins access to dashboard
  var role = "{{ 'organization_admin' | has_role }}";
  if (role == 'false') {
    document.location.href='/dashboard';
  }

  $('table').on('click', 'td[data-select-preferred-location]', function() {
    var row = $(this).parent('tr');
    var locationData = {};
    locationData.name = $(row).find('[data-attr="location_name"]').attr('data-value');
    locationData.address1 = $(row).find('[data-attr="location_address1"]').attr('data-value');
    locationData.address2 = $(row).find('[data-attr="location_address2"]').attr('data-value');
    locationData.city = $(row).find('[data-attr="location_city"]').attr('data-value');
    locationData.state = $(row).find('[data-attr="location_state"]').attr('data-value');
    locationData.zip = $(row).find('[data-attr="location_zip"]').attr('data-value');
    locationData.phone = $(row).find('[data-attr="location_phone"]').attr('data-value');
    setPreferredLocation(locationData);
  });

  $('#init-zip-search').click(function() {
    if($('#search-by-zip').val() != "")
      locationLookup($('#search-by-zip').val());
  });

  $('#reset-zip-search').click(function() {
    resetZipSearch();
  });

  $('table').on('click', '.delete-location', function() {
    var locationid = $(this).attr('data-locationid');
    var locationRow = $(this).parents('tr');
    if(confirm("Are you sure?")){
      Registria.Locations.remove(locationid, {
        success: function(){
          $(locationRow).remove();
        },
        error: function(){
