    ...
    ***************************************************/
          ...
          $(".registria_field:input[name*='location[address_attributes][country]']").rules("add", {
            required: true
                                          ...
                                          if($(".registria_field:input[name*='location[depot]']").exists()){
          $(".registria_field:input[name*='location[depot]']").rules("add", {
            required: true
          });
        }
...
function deleteLocation(locationId, locationRow) {
...
function countyLookup(county) {
  $.ajax({
    ...
    .done(function (data, status){
      populateCountyList(data)
    })
    .fail(function(e) {
      var county_autocomplete = '<span class="field-span">';
      county_autocomplete += '<div class="counties">';
      county_autocomplete += '<ul class="matching-counties">';
      county_autocomplete += '<li class="county">No counties found</li>';
      $('#county_autocomplete').html(county_autocomplete);
      $('#county_autocomplete').show();
    });
}

function populateCountyList(data) {
    var county_autocomplete = '<div class="counties">';
    county_autocomplete += '<ul class="matching-counties">';
    $.each(data.counties, function(i, v){
      var c = data.counties[i].county;
      county_autocomplete += '<li class="county"><span class="county_name">' + c.county_name + '</span> - <span class="county_state">' + c.state + '</span></li>';
    });
    $('#county_autocomplete').html(county_autocomplete);
    $('#county_autocomplete').show();
}

function setCounty(countyName, stateName) {
  $('#location_county, #location_county_lookup').val(countyName+', '+ stateName);
  //$('#zip').val($(this).find('.county_zip').text()); // Cannot set zip, am not receiving it from lookup service
  $('#state').val(stateName);
  $('#location_county').val(countyName);
  $('#name').val(countyName);
  $('#county_autocomplete').hide();
  $('#next_button').prop('disabled', null);
}

function countyLookupFieldInteraction(element) {
    $('#next_button').prop('disabled', true);
    // Without this timeout, the evaluated value will be
    // what the value was before the change that triggered this.
    setTimeout(function() {
...
$(document).ready(function() {
  ...
  $('#location_county_lookup')
    .keypress(function() {
      countyLookupFieldInteraction($(this));
    })
    ...
    .blur(function() {
      // Without this timeout, the blur event triggers before 
      // #county_autocomplete's click event and Firefox will not register the click.
      setTimeout(function() { 
        $('#county_autocomplete').hide();
      }, 200);
    });

  $('#county_lookup').on('click', '#county_autocomplete .county', function(event) {
    event.preventDefault();
    var countyName = $(this).find('.county_name').text();
    var stateName = $(this).find('.county_state').text()
    setCounty(countyName, stateName);
  });

  $('#counties_served').on('click', '.delete-location', function() {
    if(confirm('Are you sure?')) {
      var locationId = $(this).attr('data-locationid');
      var locationRow = $(this).parents('tr');
      deleteLocation(locationId, locationRow);
    }
  });
