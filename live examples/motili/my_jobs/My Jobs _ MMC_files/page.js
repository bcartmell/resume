...
$.validator.addMethod('maxSize8Mb', function(value, element) {
  if (element.files) {
    if (element.files.length>0 && element.files[0].size > 8000000)
      return false;
    else 
      return true;
  }
...
}, "file must not be larger than 8MB");
  ...
  if(confirm("Are you sure?")){
    Registria.Cases.decline(caseid, {
      success: function(){
        ...
        location.pathname == '/dashboard' ? location.reload() : location.pathname = '/dashboard';
      },
      error: function(){
        alert("We're sorry something went wrong.\nPlease try again.");
      }
...
function markJobComplete(caseId) {
  if(confirm("Are you sure?")){
...
function moveAttachmentValidationFeedback() {
  $('label.invalid[for="case_file_attachment"]').detach().insertAfter('.fileupload');
  // if ($('label.invalid[for="case_file_attachment"]').is(':visible')) {
...
function attatchmentValidationInit() {
  $('#case_file_attachment').rules('add', {
    maxSize8Mb: true
  });

  $("#registria_form").bind('invalid-form.validate', 
    function(event, validator) {
      moveAttachmentValidationFeedback();
    }
  );
}
...
$(document).ready(function() {
  $(".case-complete").live("click", function(){
    markJobComplete($(this).attr("data-caseid"));
  });

  if (document.getElementById('registria_form') && document.getElementById('case_file_attachment')) 
    attatchmentValidationInit();
});
