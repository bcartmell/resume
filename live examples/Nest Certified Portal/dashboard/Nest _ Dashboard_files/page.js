...
function openFinder2(clickSource) {

    var current_scroll_position = $(window).scrollTop() +40;

    if (clickSource) {
      var sourcePage = '/' + clickSource.attr('data-target');
      var iframe_height = clickSource.attr('data-iframe-height');
      var iframe_width = 920;
      if (clickSource.attr('data-iframe-width')) {
        iframe_width = clickSource.attr('data-iframe-width');
      }
    }
    else {
      var sourcePage = '/certified_finder';
      var iframe_height = 620;
      var iframe_width = 920;
    }

    var page_height = $('#body_wrap').height();

    if (current_scroll_position > (page_height - iframe_height - 40)) {
      current_scroll_position = current_scroll_position - iframe_height - 40;
    }
    var modal_width = parseInt(iframe_width)+40;
    var modal_width = iframe_width;

    var overlayMarkup = '<div class="overlay" style="height:' + $(document).height() + 'px;"> <div id="finder2modalWrapper" class="modalWrapper" style="width:'+ modal_width +'px; top:'+current_scroll_position+'px;"><iframe width="'+iframe_width+'" height="'+iframe_height+'" src="/certified_finder#/?show_close_button" frameborder="0" scrolling="no" style="overflow: hidden; border:none; margin-bottom:-5px;" allowfullscreen> </iframe> </div> </div>';

    $('#body_wrap').append(overlayMarkup);

    var iframe = $('iframe[src="/certified_finder#/?show_close_button"]');
    $('.overlay').has(iframe).fadeIn(400);
}
